...
import com.sql.dialect.h2.visitor.H2OutputVisitor;
import com.sql.dialect.hive.visitor.HiveOutputVisitor;
import com.sql.dialect.mysql.visitor.MySqlOutputVisitor;
import com.sql.dialect.odps.visitor.OdpsOutputVisitor;
import com.sql.dialect.oracle.visitor.OracleOutputVisitor;
import com.sql.dialect.oracle.visitor.OracleToMySqlOutputVisitor;
...
import com.sql.visitor.SchemaStatVisitor;
...
public class SQLUtils {
    ...
    private final static Logger LOG = Logger.getLogger(SQLUtils.class);
        ...
        if (name.length() > 2) {
            ...
            if ((c0 == '"' && x0 == '"') || (c0 == '`' && x0 == '`')) {
                ...
                if (JdbcConstants.ORACLE.equals(dbType)) {
                    if (Utils.isOracleKeyword(normalizeName)) {
                        return name;
                    }
                } else if (JdbcConstants.MYSQL.equals(dbType)) {
                    if (Utils.isMysqlKeyword(normalizeName)) {
                        return name;
                    }
                } else if (JdbcConstants.POSTGRESQL.equals(dbType)
                        || JdbcConstants.ENTERPRISEDB.equals(dbType)) {
                    if (Utils.isPGKeyword(normalizeName)) {
                        return name;
                    }
                }
